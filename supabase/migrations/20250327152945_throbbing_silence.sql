/*
  # Fix get_user_suggestions function

  1. Changes
    - Fix ambiguous user_id references by qualifying with table names
    - Maintain existing functionality while improving clarity
    - Ensure proper table aliases throughout the query
*/
...
BEGIN
  ...
  user_hobbies AS (
    ...
    JOIN hobbies h ON h.id = uh1.hobby_id
    WHERE uh1.user_id = user_id AND h.category = category
  ),
  potential_matches AS (
    ...
    FROM profiles p
    JOIN user_hobbies uh2 ON uh2.user_id = p.id
    JOIN hobbies h ON h.id = uh2.hobby_id
    WHERE p.id != user_id
